...
process_chunk <- function(chunk, output_folder) {
  tryCatch({
    ...
    ttops <- locate_trees(chm, lmf(ws = function(h) { 1 + 0.06 * h }, hmin = 3), uniqueness = "bitmerge")
    if (is.null(ttops) || nrow(ttops) == 0) return(NULL)

    las_trees <- segment_trees(las_veg, dalponte2016(chm, ttops, th_tree = 3))
    ...
    tree_points <- df %>%
      group_by(treeID) %>%
      summarise(X = mean(X), Y = mean(Y), Z = max(Z), .groups = "drop")
